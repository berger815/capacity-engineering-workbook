...
describe("Northstar v2 canonical fixture", () => {
  it("passes runtime schema validation and preserves supplier classification", () => {
    const parsed = capacityModelSchema.parse(northstarV2Model);
    expect(parsed.supplier).toMatchObject({ supplierId: "supplier-northstar", site: "Harbor Works" });
    expect(parsed.resourceGroups.filter(group => group.indirect).map(group => group.id)).toEqual(["rg-app", "rg-detail"]);
  });
